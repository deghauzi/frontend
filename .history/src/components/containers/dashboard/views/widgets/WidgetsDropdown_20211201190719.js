...
const WidgetsDropDown = () => {
  ...
  const mainAccountBal =
    mainAccount.length && mainAccount.map(acc => return (acc) acc)
    console.log(mainAccountBal)

  const bonusAccountBal =
    bonusAccount.length && bonusAccount.map(acc => acc);
    ...
    <CRow>
         ...
         >
           ...
           <div className="d-block pr-3">
             <h4 className="mt-2 font-weight-bold">Balance: ₦{numeral(mainAccountBal.bank_account_balance).format("0,0")}</h4>
             <p className="total-donations-received">
               {/* Account Type: {mainAccountBal.bank_account_type['name']} */}
             </p>
           </div>
         </div>
       </CCol>
